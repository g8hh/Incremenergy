...
var cnItems = {
    ...
    'WARNING: Each Hyper Upgrade level decreases the Hyper-Energy exponent by {{format(tmp.hyper.upgDecExp)}}. (Currently: -{{format(tmp.hyper.totalUpgs.times(tmp.hyper.upgDecExp))}}': '警告：每个超级升级级别都会将巨型能量指数降低 {{format(tmp.hyper.upgDecExp)}}。 （目前：-{{format(tmp.hyper.totalUpgs.times(tmp.hyper.upgDecExp))}}',
    '\t\t\tYou have': '\t\t\t你有',
    ...
    'Automated Super-Energy exponent gain': '自动化超级能量指数增益',
    ...
    'Buy max Energy Upgrades, and add 0.5 to Mega-Energy exponent for each Mega Upgrade.': '购买最大能量升级，并为每个超级升级增加 0.5 到超级能量指数。',
    'Dirt Regime': '污垢制度',
    'Energy Overflow starts 1e10x later for every Energy Upgrade level.': '能量溢出在每一次能量升级的1e10倍之后开始。',
...
var cnRegReplace = new Map([
    ...
    [/^The first 3 Energy Upgrade costs are extended by (.+) levels.$/, '前 3 次能量升级费用扩展 $1 级。'],
